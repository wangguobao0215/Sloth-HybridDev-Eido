...
> **说明**：Skill-H 健康度评分模型在父 Skill（Sloth-HybridDev-Eido）定义的 6 大核心指标（ACR、ACMR、DF、RELR、GTT、ZDC）基础上，补充了 2 个管理层视角的扩展指标——"决策记录覆盖率"和"GTM 就绪率"，用于更全面地评估研发体系健康度。扩展指标不影响父 Skill 的核心指标定义和计算逻辑。

| 指标 | 权重 | 计算公式 | 指标来源 |
|------|------|---------|---------|
| 协作协议覆盖率 | 25% | 有协议的 AI 功能数 / 总 AI 功能数 | 父 Skill 核心指标 (ACR) |
| 协议合规率 | 25% | Active 协议数 / (Active + InViolation) | 父 Skill 核心指标 (ACMR) |
| 文档鲜度 | 20% | 30 天内更新的非归档文档数 / 总非归档文档数 | 父 Skill 核心指标 (DF) |
| 需求-实验关联率 | 15% | 有关联实验的 AI 需求数 / 总 AI 需求数 | 父 Skill 核心指标 (RELR) |
| 决策记录覆盖率 | 10% | 有 ADR 的重要决策数 / 总重要决策数 | Skill-H 扩展指标 |
| GTM 就绪率 | 5% | 有 GTM 资料的已发布功能数 / 总已发布功能数 | Skill-H 扩展指标 |
