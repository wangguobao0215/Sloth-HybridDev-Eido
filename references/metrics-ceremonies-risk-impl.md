...
**回滚/重做方案（验收不通过时）**：
- Pre-commit hook 导致提交失败：回退到无 hook 版本，修复校验规则后重新部署
- SQLite 同步失败：检查 schema 兼容性，重建数据库并重新导入已有文档
- 团队成员无法独立完成创建：延长基础培训 1 周，增加 1 对 1 辅导环节
...
**回滚/重做方案（验收不通过时）**：
- 无法在 30 分钟内创建协议：简化模板字段，分阶段启用高级功能，延长试点周期 1 周
- 实验报告链接建立失败：检查 SQLite 索引一致性，修复文档 ID 生成规则
- 种子用户满意度 < 6/10：收集具体痛点，针对性优化交互和输出格式，重新培训
...
**回滚/重做方案（验收不通过时）**：
- 依赖图或健康度评分显示异常：检查数据源完整性，修复 Mermaid 渲染脚本，回退到上一稳定版本
- 销售作战卡生成失败：验证 GTM 模板库和填充逻辑，补充缺失字段的默认值
- 度量指标计算错误：核对 SQL 查询逻辑，与手工统计对比定位偏差，修正计算规则
- 培训参与率 < 90%：安排补训专场，录制视频教程供异步学习
...
**回滚/重做方案（验收不通过时）**：
- 仪式执行不到位：简化议程模板，指定仪式主持人，设置日历强制提醒
- 指标基准值缺失：检查定时任务配置，手动触发指标计算，追溯补录历史数据
- 未发现流程问题：延长观察期 1 周，引入外部视角评审（邀请其他团队观摩）
- 全体成员满意度 < 7/10：开展焦点小组访谈，识别关键阻碍，制定专项改进计划
- 团队未习惯自主使用：降低使用门槛（提供快捷命令/模板），设置每日使用提醒，将使用纳入绩效考核
